...
**コードプレフィックス規約 (軸の明確化):**
- カウント系訪問マイルストーン: `milestone_visit_count_*` (例: `milestone_visit_count_5`, `milestone_visit_count_all`)
- カウント系イベント制覇マイルストーン: `milestone_clear_count_*` (例: `milestone_clear_count_5`, `milestone_clear_count_all`)
- メタバッジ (エリア達成): `milestone_visit_areas`, `milestone_clear_areas` (count セグメントなし)

| コード例 | 件数 | 条件 | レアリティ |
| --- | --- | --- | --- |
| `milestone_visit_count_5` 〜 `milestone_visit_count_45` | 約 9 | 累計訪問ユニーク店舗数が 5/10/15/.../X | common→rare→epic |
| `milestone_visit_count_all` | 1 | 全実店舗 visited | legendary |
| `milestone_visit_areas` | 1 | 全 10 地区それぞれで 1 店舗以上 visited (メタバッジ「全国デビュー」) | epic |
| `milestone_clear_count_5` 〜 `milestone_clear_count_45` | 約 9 | イベント完了したユニーク店舗数が 5/10/15/.../X | common→rare→epic |
| `milestone_clear_count_all` | 1 | 全実店舗でイベント completed | legendary |
...
- グリッドが約 213 マスになるので、**店舗訪問・店舗別イベント達成カテゴリは 10 地区ごとにサブセクション分割** (北海道 / 埼玉・茨城・群馬・新潟 / 千葉 / 東京 / 新宿・渋谷 / 池袋 / 神奈川 / 中部 / 山陽・近畿 / 九州)。投票系は累計票数の一軸
