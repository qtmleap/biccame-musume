...
10-step ラダー: 1, 5, 10, 20, 30, ..., 570 (57 段 + 1 + 5 の計 59 エントリ)。

レアリティ配分:
- common: 1, 5, 10 (3 件)
- rare: 20–100 (9 件、10 刻み)
- epic: 110–300 (20 件、10 刻み)
- legendary: 310–570 (27 件、10 刻み)

特記:
- `event_count_50`「イベントの主」は旧 legendary → 新 rare にグレード変更 (コード安定のため rename しない)
- `event_count_570`「イベント極み」が新たな legendary ピーク (現在の総イベント数 ~189 の約 3 倍、実勢 570 を上限とした猛者向け)
...
**MVP 合計: 約 236 バッジ** (店舗訪問系 81 + イベント参加 59 + 店舗別イベント達成系 81 + 投票 17。実店舗数とビッカメ娘数で前後。メタバッジ 2 件を含む)
