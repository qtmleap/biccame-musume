...
const convertEventStores = async () => {
  ...
  for (const event of events) {
    ...
    if (unconvertedStores.length > 0) {
      console.warn(`Event "${event.name}": Could not convert stores:`, unconvertedStores)
    }
    ...
    // イベントを更新
    console.log(`Updating event "${event.name}": ${event.stores.join(', ')} -> ${convertedStores.join(', ')}`)

    try {
      const updateResponse = await fetch(`http://localhost:5173/api/events/${event.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!updateResponse.ok) {
        console.error(`Failed to update event "${event.name}": ${updateResponse.statusText}`)
        skippedCount++
      } else {
        updatedCount++
      }
