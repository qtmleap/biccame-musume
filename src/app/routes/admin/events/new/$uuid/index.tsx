 ...
 */
const EventNewWithIdSearchSchema = z.object({
...
/**
 * イベント新規作成画面のコンテンツ（UUID指定版）
 */
const NewEventWithIdContent = () => {
  const router = useRouter()
  const { uuid } = useParams({ from: '/admin/events/new/$uuid/' })
  const search = useSearch({ from: '/admin/events/new/$uuid/' })
 ...
 * イベント新規作成ページ（UUID指定版）
 */
const NewEventWithIdPage = () => {
  return (
    <Suspense fallback={<LoadingFallback />}>
      <NewEventWithIdContent />
    </Suspense>
  )
}

export const Route = createFileRoute('/admin/events/new/$uuid/')({
  component: NewEventWithIdPage,
  validateSearch: EventNewWithIdSearchSchema
})
